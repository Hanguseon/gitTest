...
public class ProfessorController {
	...
	public ModelAndView ProfAddOk(Locale locale,Model model) {
		...
		if (deptno == 0) {
			...
			return web.redirect(null, "학과를 입력하세요");
		}

		/**3)저장을 위한 JavaBeans구성하기 */
	...
	public ModelAndView ProfEditOk(Locale locale,Model model) {
		...
		if(name == null){
			return web.redirect(null, "교수이름이 없습니다.");
		}
		if(userid==null){
			return web.redirect(null, "교수아이디가 없습니다.");
		}
		if(position==null){
			return web.redirect(null, "교수직급이 없습니다.");
		}
		if(sal==0){
			return web.redirect(null, "교수급여가 없습니다.");
		}
		if(hiredate==null){
			return web.redirect(null, "교수입사일이 없습니다.");
		}
		if(deptno==0){
			return web.redirect(null, "교수학과이름이 없습니다.");
		}
