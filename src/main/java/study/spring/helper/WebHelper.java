...
public class WebHelper {
	...
	public void init() {
		...
		/**JSP내장객체를 담고있는 Spring의 객체를 통해서 내장객체 획ㄱ득하기 */
		//-->import org.springframework.web.context.request.RequestContextHolder;
		//-->import org.springframework.web.context.request.ServletRequestAttribute;
		ServletRequestAttributes requestAttr
			= (ServletRequestAttributes) RequestContextHolder.getRequestAttributes();
		
		//request와 response객체를 추출한다.
	...
	public String convertHtmlTag(String content) {
		...
		for(int i=0; i<content.length(); i++) {
			...
			//특수문자 형태에 부합할 경우 변환하여 builder에 추가,그렇지 않을 경우 원본 그대로 builder에 추가
			switch(chrBuff) {
			case '<':
				builder.append("&lt;");
				break;
